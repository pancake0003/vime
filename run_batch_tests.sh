...
TESTS=(
  ...
  "test_gemma4_12B_gsm8k_short.py"
  "test_mimo_7B_mtp_only_grad.py"
  "test_moonlight_16B_A3B.py"
  "test_moonlight_16B_A3B_r3.py"
)
LOG_DIR="$HOME/logs"
mkdir -p "$LOG_DIR"
RUN_TS=$(date +%Y%m%d_%H%M%S)
SUMMARY="$LOG_DIR/summary_${RUN_TS}.txt"
...
echo "Batch run $RUN_TS | timeout=${TIMEOUT_SECS}s | ${#TESTS[@]} tests" | tee "$SUMMARY"

for t in "${TESTS[@]}"; do
  LOG_FILE="$LOG_DIR/${t%.py}_${RUN_TS}.log"
  CNAME="vime-${t%.py}"
  echo "=== $(date) Running: $t (log: $LOG_FILE) ==="
  # --name lets us kill the exact container on timeout. `timeout` sends the
  # docker CLI a signal; we also explicitly `docker kill` so the daemon-side
  # container actually dies rather than orphaning.
